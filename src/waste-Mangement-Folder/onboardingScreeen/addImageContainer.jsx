...
function AddImageContainer() {
  ...
  const {selectedImage, setSelectedImage} =UserAuth();
  ...
  const handleCreateContainer = async () => {
    if (selectedImage) {
      setIsLoading(true);
      try {
        // Store the selected image in the "location" collection
        const locationsRef = collection(db, "location");
        const docRef = await addDoc(locationsRef, { image: selectedImage });

        // Now you can create a new container document that references the image
        // Replace this with your actual container creation logic
        // const containerData = {
        //   name: "YourContainerName",
        //   imageRef: docRef.id, // Reference to the stored image
        //   otherContainerData: "YourContainerData",
        // };
        ...
        <div className="image-upload-box">
          <label className="add-image-label" htmlFor="image-upload">
            <input
              ...
              ref={fileInputRef} // Set the file input ref
            />
